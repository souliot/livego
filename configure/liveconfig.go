...
func initLog() {
	if l, err := ParseLevel(Config.GetString("level")); err == nil {
		logs.SetLevel(l)
	} else {
...
func ParseLevel(lvl string) (int, error) {
	...
	case "info":
		return logs.LevelInfo, nil
	case "trace":
		return logs.LevelTrace, nil
	case "error":
		return logs.LevelError, nil
	case "warn", "warning":
		return logs.LevelWarn, nil
	case "notice":
	...
	return -1, fmt.Errorf("not a valid logrus Level: %q", lvl)
}
func init() {
	defer Init()
	// Default config
	...
	c := ServerCfg{}
	Config.Unmarshal(&c)
	logs.Debug("Current configurations: \n%# v", pretty.Formatter(c))
}
