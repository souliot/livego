...
func JWTMiddleware(next http.Handler) http.Handler {
	...
	logs.Info("Using JWT middleware")
			...
			if s.GetReader() != nil {
				...
				case *rtmp.VirReader:
					...
					msg := stream{item.Key, v.Info().URL, v.ReadBWInfo.StreamId, v.ReadBWInfo.VideoDatainBytes, v.ReadBWInfo.VideoSpeedInBytesperMS,
						v.ReadBWInfo.AudioDatainBytes, v.ReadBWInfo.AudioSpeedInBytesperMS, v.Alive()}
				...
				if pw.GetWriter() != nil {
					...
					case *rtmp.VirWriter:
						...
						msg := stream{item.Key, v.Info().URL, v.WriteBWInfo.StreamId, v.WriteBWInfo.VideoDatainBytes, v.WriteBWInfo.VideoSpeedInBytesperMS,
							v.WriteBWInfo.AudioDatainBytes, v.WriteBWInfo.AudioSpeedInBytesperMS, v.Alive()}
...
func (s *Server) handlePull(w http.ResponseWriter, req *http.Request) {
	...
	logs.Debug("control pull: oper=%v, app=%v, name=%v, url=%v", oper, app, name, url)
	...
	if oper == "stop" {
		...
		logs.Debug("rtmprelay stop push %s from %s", remoteurl, localurl)
		pullRtmprelay.Stop()
		...
		res.Status = 400
		res.Data = retString
		logs.Debug("pull stop return %s", retString)
	} else {
		pullRtmprelay := rtmprelay.NewRtmpRelay(&localurl, &remoteurl)
		logs.Debug("rtmprelay start push %s from %s", remoteurl, localurl)
		...
		res.Status = 400
		res.Data = retString
		logs.Debug("pull start return %s", retString)
	}
}
...
func (s *Server) handlePush(w http.ResponseWriter, req *http.Request) {
	...
	logs.Debug("control push: oper=%v, app=%v, name=%v, url=%v", oper, app, name, url)
	if (len(app) <= 0) || (len(name) <= 0) || (len(url) <= 0) {
		res.Data = "control push parameter error, please check them."
		return
	}
	...
	if oper == "stop" {
		...
		retString = fmt.Sprintf("<h1>push url stop %s ok</h1></br>", url)
		res.Data = retString
		logs.Debug("push stop return %s", retString)
	} else {
		...
		res.Data = retString
		logs.Debug("push start return %s", retString)
	}
}
