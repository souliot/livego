...
func NewSource(info av.Info) *Source {
	...
	go func() {
		err := s.SendPacket()
		if err != nil {
			logs.Warning("send packet error: ", err)
			s.closed = true
		}
	}()
	return s
...
func (source *Source) DropPacket(pktQue chan *av.Packet, info av.Info) {
	logs.Warning("[%v] packet queue max!!!", info)
...
func (source *Source) SendPacket() error {
	defer func() {
		logs.Debug("[%v] hls sender stop", source.info)
		if r := recover(); r != nil {
			logs.Warning("hls SendPacket panic: ", r)
		}
	}()

	logs.Debug("[%v] hls sender start", source.info)
	for {
		if source.closed {
...
func (source *Source) Close(err error) {
	logs.Debug("hls source closed: ", source.info)
